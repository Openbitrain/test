...
async function fetchJob_list(index) {
  ...
  const apiUrl = `https://www.linkedin.com/jobs-guest/jobs/api/seeMoreJobPostings/search?keywords=developer%20or%20engineer&geoId=103644278&f_TPR=r86400&f_WT=2&start=${index}&sortBy=DD`;
...
app.get('/time', async (req, res) => {
  ...
  const apiUrl = `https://www.linkedin.com/jobs-guest/jobs/api/seeMoreJobPostings/search?keywords=developer%20or%20engineer&geoId=103644278&f_TPR=r86400&f_WT=2&start=0`;
  let headers = {
    "User-Agent": userAgents[Math.floor(Math.random() * userAgents.length)],
    Accept: "application/json, text/javascript, */*; q=0.01",
    "Accept-Language": "en-US,en;q=0.9",
  ...
  res.json({ "re": await response.data });
})
